...
func main() {
	...
	mux.HandleFunc("/set-location", func(w http.ResponseWriter, r *http.Request) {
		...
		_, err := db.Exec(
			"INSERT INTO geofire_locations(path, location_key, lat, lng, geom, data) "+
				"VALUES(?, ?, ?, ?, ST_SRID(POINT(?, ?), 4326), ?) "+
				"ON DUPLICATE KEY UPDATE "+
				"lat = VALUES(lat), lng = VALUES(lng), geom = VALUES(geom), data = VALUES(data), updated_at = CURRENT_TIMESTAMP",
			req.Path, req.ID, req.Lat, req.Lng, req.Lng, req.Lat, string(dataBytes),
		)
		if err != nil {
	...
	mux.HandleFunc("/remove-location", func(w http.ResponseWriter, r *http.Request) {
		...
		if _, err := db.Exec("DELETE FROM geofire_locations WHERE path=? AND location_key=?", req.Path, req.ID); err != nil {
			http.Error(w, err.Error(), http.StatusInternalServerError)
			return
		}
		jsonResponse(w, http.StatusOK, map[string]any{"ok": true})
	...
	mux.HandleFunc("/get-location", func(w http.ResponseWriter, r *http.Request) {
		...
		var dataBytes []byte
		err := db.QueryRow(
			"SELECT lat, lng, data FROM geofire_locations WHERE path=? AND location_key=? LIMIT 1",
			path, id,
	...
	mux.HandleFunc("/query", func(w http.ResponseWriter, r *http.Request) {
		...
		rows, err := db.Query(
			"SELECT location_key, lat, lng, data FROM geofire_locations "+
				"WHERE path = ? AND ST_Distance_Sphere(geom, ST_SRID(POINT(?, ?), 4326)) <= ?",
	...
	log.Printf("geofire-mysql-starter listening on :%s", port)
